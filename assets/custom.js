...
$('a.smooth-scroll').on('click', function(event) {
    ...
    event.preventDefault();
      ...
      // console.log( $anchor[0].hash);
      // let byClass = document.getElementsByClassName('current');
      // byClass[0].className= "tab-title";
      // $($anchor[0].hash).addClass('current')
...
function subscribeEmail(){
    var txt=$("#subsForm button").text();
    if(txt=='Processing...'){
        return;
    }
    var emailId=$("#m3Email").val();
    if(emailId==""){
        return;
    }
    $("#subsForm button").text("Processing...");
    $.ajax({
        url:'./subscribe-email',
        type:'post',
        data:{email:emailId},
        success:function(res){
            res=JSON.parse(res);
            $("#subsForm").html('<p><b>'+res.msg+'</b></p>')
        },
        error:function(err){
            $("#subsForm button").text("SUBSCRIBE Again");
        }
    })
}